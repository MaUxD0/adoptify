...
const adoptionStatusValues = [
  ...
  AdoptionStatuses.CANCELLED,
] as const;

const adoptionStatusUpdateValues = [AdoptionStatuses.APPROVED, AdoptionStatuses.REJECTED] as const;

export const createAdoptionSchema = z.object({
  petId: z.string().cuid({ message: 'Invalid pet ID' }),
  shelterId: z.string().cuid({ message: 'Invalid shelter ID' }),
...
export const adoptionIdParamSchema = z.object({
  id: z.string().cuid({ message: 'Invalid adoption ID' }),
});
...
export type AdoptionFiltersInput = z.infer<typeof adoptionFiltersSchema>;
